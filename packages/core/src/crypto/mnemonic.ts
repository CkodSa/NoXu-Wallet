...
// Ledger uses the updated coin type 111111 (standard for Kaspa Ledger app)
export const LEDGER_DERIVATION_PATH = "m/44'/111111'/0'/0/0";
...
/**
 * Create a HardwareAccount from a public key received from a Ledger device.
 */
export function createHardwareAccount(
  publicKey: Uint8Array,
  derivationPath: string,
  isTestnet = false
): HardwareAccount {
  const address = pubkeyToKaspaAddress(publicKey, isTestnet);
  return {
    publicKey,
    address,
    derivationPath,
