...
function InnerApp() {
  ...
  const [tokenPrice, setTokenPrice] = useState<{ price: number; change_24h?: number; volume?: number; image?: string } | null>(null);
  const [tokenPriceLoading, setTokenPriceLoading] = useState(false);
  const [tokenPriceHistory, setTokenPriceHistory] = useState<number[]>([]);
  const [tokenChartLoading, setTokenChartLoading] = useState(false);
    ...
    try {
      ...
      if (res?.ok && res.price) {
        setTokenPrice({
          ...
          image: res.price.image,
        });
        // Update token logo if image available and not already set
        if (res.price.image && activeToken?.kind === "krc20" && !activeToken.logoURI) {
          setActiveToken({ ...activeToken, logoURI: res.price.image });
        }
      }
    } catch {
  ...
  // Fetch KRC-20 token chart data
  const loadTokenChart = async (symbol: string, range: "D" | "W" | "M") => {
    setTokenChartLoading(true);
    try {
      const res = await rpc("GET_TOKEN_PRICE_HISTORY", { symbol, days: chartDaysMap[range] });
      if (res?.ok && res.history) {
        const prices = res.history.map((p: any) => p.price);
        setTokenPriceHistory(prices);
      } else {
    ...
    setTokenChartLoading(false);
  };

  // Refetch chart when range changes
  useEffect(() => {
    if (mainPage !== "token" || !activeToken) return;
    if (activeToken.symbol === "KAS") {
      loadKasPrice(chartRange);
    } else if (activeToken.kind === "krc20") {
      loadTokenChart(activeToken.symbol, chartRange);
    }
  ...
  // Build image lookup from trending/popular tokens + current price response
  const tokenImageMap: Record<string, string> = {};
  for (const t of [...trendingTokens, ...trendingGainers]) {
    if (t.image) tokenImageMap[t.symbol.toUpperCase()] = t.image;
  }
  // Include image from the active token's price fetch
  if (tokenPrice?.image && activeToken?.kind === "krc20") {
    tokenImageMap[activeToken.symbol.toUpperCase()] = tokenPrice.image;
  }

  // Build dynamic token list from fetched KRC-20 balances (only tokens with non-zero balance)
  const krc20Tokens: TokenMeta[] = (tokenBalances || [])
    .filter((tb: TokenBalance) => BigInt(tb.balance) > 0n)
    .map((tb: TokenBalance) => ({
      id: `krc20_${tb.tick}`,
      ...
      logoURI: tokenImageMap[tb.tick.toUpperCase()],
    }));
                ...
                {pnlSummary && pnlBackfillComplete ? (() => {
                  ...
                  return (
                    <div className={`pnl-row ${isUp ? "up" : "down"}`}>
                      <span className="pnl-label"><PnlIcon /> PnL</span>
                  ...
                  <div className={`pnl-row ${kasPriceChange >= 0 ? "up" : "down"}`}>
                    <span className="pnl-label"><PnlIcon /> 24h PnL</span>
                    <span className="pnl-value">
  ...
  const TokenDetailCard = activeToken && (
    <div className="card token-detail">
      ...
      {activeToken.kind === "krc20" && (
        <>
          ...
          <div className="token-chart">
            {tokenChartLoading && tokenPriceHistory.length === 0 ? (
              <div className="chart-loading">
                <span className="muted small">Loading chart...</span>
              </div>
            ) : tokenPriceHistory.length > 0 ? (
              <TokenChart data={tokenPriceHistory} positive={(tokenPrice?.change_24h ?? 0) >= 0} />
            ) : (
              <div className="chart-loading">
      ...
      {/* Token Info Section for KAS */}
      {activeToken.symbol === "KAS" && kasPrice != null && (
        <div className="token-info-section">
          <div className="token-info-grid">
            <div className="token-info-item">
              <span className="token-info-label">Price</span>
              <span className="token-info-value">
                {currencySymbol}{kasPrice.toLocaleString(undefined, { minimumFractionDigits: 4, maximumFractionDigits: 6 })}
              </span>
            </div>
            <div className="token-info-item">
              <span className="token-info-label">24h Change</span>
              <span className="token-info-value" style={{ color: kasPriceChange >= 0 ? "var(--success)" : "var(--danger)" }}>
                {kasPriceChange >= 0 ? "+" : ""}{kasPriceChange.toFixed(2)}%
              </span>
            </div>
            <div className="token-info-item">
              <span className="token-info-label">Network</span>
              <span className="token-info-value">{network === "mainnet" ? "Mainnet" : "Testnet"}</span>
            </div>
          </div>
        </div>
      )}
        ...
        {filteredTokens.map(({ token, amount }) => (
          <button
            ...
            onClick={() => {
              ...
              setMainPage("token");
              // Load token info + price + chart for KRC-20 tokens
              setChartRange("W");
              if (token.kind === "krc20") {
                loadTokenInfo(token.symbol);
                loadTokenPrice(token.symbol);
                setTokenPriceHistory([]);
                loadTokenChart(token.symbol, "W");
              } else {
                setTokenInfo(null);
                setTokenPrice(null);
                setTokenPriceHistory([]);
          ...
          >
            <div className="token-row-left">
              {token.logoURI ? (
                <img className="token-icon-img" src={token.logoURI} alt={token.symbol} />
              ) : (
                <div className="token-icon-letter">{token.symbol.charAt(0).toUpperCase()}</div>
              )}
        ...
        {activePopularList.length > 0 ? activePopularList.map((t: any) => (
          <button
            ...
            onClick={() => {
              ...
              loadTokenChart(t.symbol, "W");
              contentRef.current?.scrollTo(0, 0);
            }}
          >
            <div className="token-row-left">
              {t.image ? (
                <img className="token-icon-img" src={t.image} alt={t.symbol} />
              ) : (
                <div className="token-icon-letter">{t.symbol.charAt(0).toUpperCase()}</div>
              )}
              <div className="token-main">
        ...
        {mainPage === "home" && (
          <ScreenLayout title="Home">
            ...
            <div className="quick-actions">
              <button className="quick-action" onClick={() => { setSelectedToken("KAS"); setMainPage("send"); }}>
                <div className="quick-action-icon">
                  <span className="quick-action-arrow">↑</span>
                </div>
                <span className="quick-action-label">Send</span>
              </button>
              <button className="quick-action" onClick={() => setMainPage("receive")}>
                <div className="quick-action-icon">
                  <span className="quick-action-arrow">↓</span>
                </div>
                <span className="quick-action-label">Receive</span>
              </button>
              <button className="quick-action" onClick={() => { loadPnlDetail(); setMainPage("pnl"); }}>
                <div className="quick-action-icon">
                  <PnlIcon />
                </div>
                <span className="quick-action-label">PnL</span>
              </button>
            </div>
            {/* Watch-Only Preview */}
            {securityFeatures?.watchOnlyAddresses &&
